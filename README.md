...
1. 訊號合成原理：模擬訊號由主頻率正弦波與隨機雜訊組成：  
   ![公式](https://latex.codecogs.com/svg.latex?x(t)=A\sin(2\pi ft)+N(\mu,\sigma))  
   其中 N(μ,σ) 代表平均值為 μ、標準差為 σ 的常態分佈雜訊 (Gaussian Noise)。  

2. 快速傅立葉變換 (FFT)：為了分析頻率成分，我們將離散時域訊號 x[n] 轉換為頻域訊號 X[k]：  
   ![公式](https://latex.codecogs.com/svg.latex?X[k]=\sum_{n=0}^{N-1}x[n]e^{-j\frac{2\pi}{N}kn})  
   這讓我們能從雜亂的時域波形中，分離出特定頻率的訊號與寬頻雜訊。  

3. 頻域濾波 (Frequency Domain Filtering)：低通濾波器的運作原理是對頻譜套用一個遮罩 (Mask) H[k]：  
   ![公式](https://latex.codecogs.com/svg.latex?Y[k]=X[k]\cdot H[k])  
   其中截止頻率 (Cutoff Frequency) fc 定義了遮罩範圍：  
   ![公式](https://latex.codecogs.com/svg.latex?H[k]=\begin{cases}1,&|f|\leq f_c\text{(保留低頻)}\\0,&|f|>f_c\text{(濾除高頻)}\end{cases})  
